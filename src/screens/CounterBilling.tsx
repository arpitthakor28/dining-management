...
export default function CounterBilling() {
  ...
  const [todaySales, setTodaySales] = useState<any>({ totalRevenue: 0, billsCount: 0 });
  const [selectedTableId, setSelectedTableId] = useState<string>('');
  const [pdfDownloadUrl, setPdfDownloadUrl] = useState<string | null>(null);

  // Tab State
  const [activeTab, setActiveTab] = useState<'billing' | 'kitchen' | 'tables' | 'menu'>('billing');
    ...
    try {
      ...
      if (tablesRes.ok) {
        ...
        if (!selectedTableId && tData.length > 0) {
          const activeTable = tData.find((t: any) => t.status !== 'empty');
  ...
  const handleResolveHelp = async (requestId: number) => {
    try {
  ...
  const handleCancelBillRequest = async (sessionId: string) => {
    if (!window.confirm("Are you sure you want to cancel this bill request? This will unlock ordering for the table.")) return;
  ...
  const handleConfirmBill = async (sessionId: string) => {
    try {
  ...
  // Group items maps
  const groupedItemsMap = tableOrders.reduce((acc: any, order: any) => {
    ...
    if (order.notes) acc[itemId].notes.push(order.notes);
    return acc;
  }, {} as Record<string, any>);

  const allOrderedItems = Object.values(groupedItemsMap) as any[];
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 bg-white/5 backdrop-blur-md p-6 rounded-2xl border border-white/10 shadow-lg">
          ...
          <div className="flex flex-wrap gap-2 mt-4 md:mt-0">
             ...
             <button 
               ...
               className="px-4 py-2.5 btn-premium-danger rounded-lg transition text-sm flex items-center gap-1.5 ml-2"
             >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-200">
              ...
              <div className="glass-card rounded-xl border border-white/10 overflow-hidden" onMouseMove={handleMouseMove}>
                ...
                <div className="p-4 space-y-4 max-h-[350px] overflow-y-auto z-10">
                  ...
                  <div>
                    ...
                    {staffCalls.length === 0 ? (
                      <p className="text-xs text-gray-400 italic py-2">No active staff calls</p>
                    ) : (
                      <div className="space-y-2">
                        {staffCalls.map(hr => (
                          <div key={hr.id} className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg flex justify-between items-center animate-in slide-in-from-top-1">
                            <span className="font-bold text-sm text-blue-200">Table {hr.table_number} requests help</span>
                            <button
                              onClick={() => handleResolveHelp(hr.id)}
                              className="btn-premium-green text-white font-bold p-1.5 rounded transition-all active:scale-95"
                              title="Resolve call"
                            >
                  ...
                  <div className="border-t border-white/10 pt-3">
                    ...
                    {billRequests.length === 0 ? (
                      <p className="text-xs text-gray-400 italic py-2">No pending bill requests</p>
                    ) : (
                      <div className="space-y-2">
                        {billRequests.map(hr => (
                          <div 
                            key={hr.id} 
                            onClick={() => setSelectedTableId(hr.table_id)}
                            className={`p-3 border rounded-lg flex justify-between items-center cursor-pointer transition-all ${
                              selectedTableId === hr.table_id 
                  ...
                  {tables.map(table => {
                    ...
                    return (
                      <div 
                        ...
                        className={`p-3.5 border rounded-xl flex justify-between items-center transition-all cursor-pointer ${
                          isSelected 
            ...
            <div className="glass-card rounded-xl border border-white/10 lg:col-span-2 h-[80vh] flex flex-col overflow-hidden" onMouseMove={handleMouseMove}>
               ...
               <div className="flex justify-between items-start border-b border-white/10 p-6 bg-white/5 z-10">
                    <div>
                      <h2 className="text-2xl font-black text-white flex items-center gap-3">
                        ...
                        {isBillRequested && (
                          <span className="bg-orange-500/20 text-orange-300 border border-orange-500/30 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-sm animate-pulse">
                            Bill Requested
                          </span>
                        )}
                      </h2>
               ...
               <div className="flex-1 overflow-y-auto w-full p-4 scrollbar-glass z-10">
                          ...
                          allOrderedItems.map((item, idx) => (
                            <tr key={idx} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                              <td className="py-4 px-6">
                                ...
                                {item.notes.length > 0 && item.notes.filter(Boolean).map((note: string, nIdx: number) => (
                                  <span key={nIdx} className="block text-xs text-red-300 font-semibold mt-1">Note: {note}</span>
                                ))}
                              </td>
                      ...
                      <div className="mt-8 border-t border-white/10 pt-4">
                        ...
                        <div className="space-y-2">
                          {tables.find(t => t.id === selectedTableId)?.comments ? (
                            <div className="p-3 bg-yellow-500/10 border border-yellow-500/20 text-yellow-350 rounded-lg text-sm font-semibold">
                              "{tables.find(t => t.id === selectedTableId).comments}"
                            </div>
                          ) : (
                            <p className="text-xs text-gray-555 italic">No notes sent by guest</p>
                          )}
                        </div>
                      </div>
                    )}
                    ...
                    {allOrderedItems.length > 0 && (
                      <div className="flex justify-end p-8 w-full border-t border-dashed border-white/10 mt-6 bg-white/5">
               ...
               <div className="border-t border-white/10 p-4 bg-white/5 flex gap-4 mt-auto z-10">
                    {pdfDownloadUrl && (
                      <a 
                        href={pdfDownloadUrl} 
                        download
                        className="px-6 py-3 btn-premium-outline rounded-lg font-bold transition flex items-center justify-center gap-2 hover:border-green-500 hover:text-green-400"
                      >
                    ...
                    {isBillRequested && tableSessionId && (
                      <button 
                        ...
                        className="px-6 py-3 btn-premium-danger rounded-lg transition flex items-center justify-center gap-2"
                      >
                    ...
                    <button 
                      ...
                      className={`px-8 py-3 rounded-lg font-bold flex items-center gap-2 justify-center transition-all sm:w-auto w-full ${
                        allOrderedItems.length === 0 
                ...
                {batches.filter(b => b.items.some(i => i.status !== 'served')).map((batch) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`glass-card rounded-2xl overflow-hidden flex flex-col min-h-[300px] border transition-all duration-300 ${
                        isUrgent ? 'border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.15)]' : 'border-white/10'
                      }`}
                    >
                      {/* Ticket Header */}
                      <div className={`px-4 py-3 border-b flex justify-between items-center z-10 ${
                        isUrgent ? 'bg-red-500/10 border-red-500/20' : 'bg-white/5 border-white/10'
                      }`}>
                        <div>
                        ...
                        <span className={`text-xs font-bold flex items-center gap-1 px-2.5 py-1 rounded-lg ${
                          isUrgent ? 'bg-red-500/20 text-red-300 animate-pulse' : 'bg-white/10 text-gray-300'
                        }`}>
                            ...
                            <div key={idx} className={`py-3 flex flex-col gap-2 ${
                              ...
                              <div className="flex justify-between items-start">
                                ...
                                <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full uppercase ${
                                  item.status === 'served' ? 'bg-white/10 text-gray-400' :
                              ...
                              {item.status !== 'served' && (
                                <div className="flex gap-1.5 mt-1">
                                  {item.status === 'pending' && (
                                    <button
                                      onClick={() => updateItemStatus(item.itemId, 'preparing')}
                                      className="flex-1 btn-premium-blue py-1.5 px-2 text-xs rounded-lg transition-all flex items-center justify-center gap-1 active:scale-95"
                                    >
                                  ...
                                  {item.status === 'preparing' && (
                                    <button
                                      onClick={() => updateItemStatus(item.itemId, 'ready')}
                                      className="flex-1 btn-premium-amber py-1.5 px-2 text-xs rounded-lg transition-all flex items-center justify-center gap-1 active:scale-95"
                                    >
                                      <Check size={12} /> Ready
                                    </button>
                                  )}
                                  {item.status === 'ready' && (
                                    <button
                                      onClick={() => updateItemStatus(item.itemId, 'served')}
                                      className="flex-1 btn-premium-green py-1.5 px-2 text-xs rounded-lg transition-all flex items-center justify-center gap-1 active:scale-95"
                                    >
                          ...
                          <div className="flex-1 text-center md:text-left min-w-0">
                            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-1.5">
                              ...
                              <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase ${
                                table.status === 'bill_requested' ? 'bg-orange-500/20 text-orange-355 border border-orange-500/30' :
                ...
                <form onSubmit={handleAddMenuItem} className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="input-dark w-full rounded-lg py-2.5 px-4 font-bold text-sm bg-[#0d120e] text-[#f1f5f2]"
                    >
                      {CATEGORIES.map(cat => (
                        <option key={cat.id} value={cat.id} className="bg-[#0d120e] text-[#f1f5f2]">{cat.name}</option>
                      ))}
                    </select>
                  </div>
                              ...
                              <td className="py-4 px-4">
                                {isEditing ? (
                                  <select
                                    ...
                                    className="input-dark rounded py-1 px-2 text-xs font-bold bg-[#0d120e]"
                                  >
                                    {CATEGORIES.map(cat => (
                                ...
                                {isEditing ? (
                                  <div className="flex items-center gap-1 justify-end">
                                    ...
                                    <input
                                      ...
                                      className="input-dark w-20 rounded py-1 px-2 text-xs font-bold text-right"
                                    />
                                  </div>
                                ) : (
                                  ...
                                  {isEditing ? (
                                    <>
                                      <button
                                        ...
                                        className="p-1.5 btn-premium-green rounded transition"
                                        title="Save Changes"
                                      >
                                        <Save size={14} />
                                      </button>
                                      <button
                                        ...
                                        className="p-1.5 btn-premium-danger rounded transition"
                                        title="Cancel"
                                      >
                                        <XCircle size={14} />
                                      </button>
                                    ...
                                    <>
                                      <button
                                        ...
                                        className="p-1.5 btn-premium-outline hover:border-green-500 hover:text-green-400 rounded transition"
                                        title="Edit Item"
                                      >
                                        <Edit2 size={14} />
                                      </button>
