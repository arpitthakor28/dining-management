...
export default function SharedCart() {
  ...
  return (
    <div className="pb-32 pt-4 px-4 bg-gray-50 h-full relative" style={{ minHeight: '100vh' }}>
      ...
      {/* Bill Locked banner */}
      {isOrderLocked && (
        <div className="mb-4 bg-orange-50 border border-orange-200 text-orange-850 p-3.5 rounded-xl flex items-center gap-2 shadow-sm text-xs font-bold">
      ...
      <div className="space-y-4 mb-4">
        {items.length === 0 ? (
          ...
          items.map(item => (
            <div key={item.id} className="card p-4 shadow-sm border-l-4 border-l-green-500 bg-white">
              ...
              {item.notes && (
                <div className="bg-gray-50 border border-gray-200 p-2 rounded-lg mb-3">
              ...
              <div className="flex justify-between items-center mt-4">
                <button 
                  onClick={() => !isOrderLocked && removeFromCart(item.id)}
                  disabled={isOrderLocked}
                  className={`text-xs flex items-center gap-1 font-bold py-1 px-2 rounded -ml-2 ${
                    isOrderLocked ? 'text-gray-300 cursor-not-allowed' : 'text-red-500 active:bg-red-50'
                  }`}
                >
                ...
                <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-md px-2 py-1 shadow-sm">
                  <button 
                    onClick={() => !isOrderLocked && updateQuantity(item.id, item.quantity - 1)} 
                    disabled={isOrderLocked}
                    className="p-1 active:bg-gray-100 rounded text-gray-600 disabled:opacity-55"
                  >
                    <Minus size={16} />
                  </button>
                  ...
                  <button 
                    onClick={() => !isOrderLocked && updateQuantity(item.id, item.quantity + 1)} 
                    disabled={isOrderLocked}
                    className="p-1 active:bg-gray-100 rounded text-primary disabled:opacity-55"
                  >
                    <Plus size={16} />
                  </button>
                </div>
      ...
      <div className="fixed bottom-0 w-full max-w-[480px] bg-white border-t border-gray-200 p-4 pb-6 left-1/2 transform -translate-x-1/2 rounded-t-xl z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        ...
        <button 
          className={`w-full font-bold py-3 text-lg rounded-xl shadow-md ${
            cartTotal > 0 && !isOrderLocked 
