...
export default function RunningBill() {
    ...
    const { batches, isBillRequested, requestBill, sessionClosed, clearTableSession } = useCart();
    ...
    return (<div className="pb-24 pt-4 px-4 bg-gray-50 h-full relative" style={{ minHeight: '100vh' }}>
      ...
      {isBillRequested && (<div className="mb-6 bg-green-50 border border-green-200 text-green-800 p-4 rounded-lg flex items-center gap-3 shadow-sm animate-in fade-in zoom-in slide-in-from-top-2">
      ...
      <div className="card p-0 overflow-hidden shadow-sm border border-gray-200 mb-6 font-mono text-sm bg-white">
        ...
        <div className="p-4 space-y-3 pb-6 border-b border-gray-100 border-dashed min-h-[100px]">
          {allOrderedItems.length === 0 ? (<p className="text-gray-400 text-center font-sans">No items ordered yet.</p>) : (allOrderedItems.map((item, idx) => (<div key={idx} className="flex justify-between items-start">
                <div className="w-3/4">
      ...
      <button className={`w-full font-bold py-3 text-lg rounded-xl shadow-md ${isBillRequested || subtotal === 0 ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-primary text-white hover:bg-primary-dark active:scale-95 transition-all'}`} onClick={handleRequestBill} disabled={isBillRequested || subtotal === 0}>
        {isBillRequested ? 'Bill Requested' : 'Request Final Bill'}
      </button>

      {showConfirm && (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
