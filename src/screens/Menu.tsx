...
export default function Menu() {
  ...
  const handleTabChange = (tabName: 'menu' | 'orders' | 'bill') => {
    if (tabName === 'menu') navigate(`/table/${tableId}/menu${window.location.search}`);
    else if (tabName === 'orders') navigate(`/table/${tableId}/status${window.location.search}`);
    else if (tabName === 'bill') navigate(`/table/${tableId}/bill${window.location.search}`);
  };
  ...
  const handlePlaceOrder = async () => {
    if (cartItems.length === 0 || isOrderLocked) return;
  ...
  if (isValidating) {
    return (
      <div className="light-green-wrapper flex flex-col items-center justify-center p-6 min-h-screen text-slate-800">
  ...
  if (!isValid) {
    return (
      <div className="light-green-wrapper flex flex-col items-center justify-center p-6 text-center min-h-screen text-slate-800">
  ...
  if (sessionClosed) {
    return (
      <div className="light-green-wrapper flex flex-col items-center justify-center p-6 text-center min-h-screen text-slate-800">
        <div className="bg-white rounded-3xl border border-gray-200 p-8 max-w-sm shadow-lg flex flex-col items-center">
          <div 
            className="rounded-full flex items-center justify-center mb-6 text-[#16a34a]"
            style={{
              width: '80px',
              height: '80px',
  ...
  return (
    <div className="light-green-wrapper pb-28 pt-16 relative bg-[#f4f7f5] text-slate-800 min-h-screen">
      ...
      <header className="fixed top-0 left-0 w-full bg-white border-b border-gray-200/80 py-3.5 px-6 flex justify-between items-center z-40 shadow-sm">
        ...
        <div className="hidden md:flex items-center gap-6">
          <button 
            ...
            className={`guest-tab-btn text-sm uppercase tracking-wider font-extrabold transition-all ${
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-6 mt-4">
        ...
        {isOrderLocked && (
          <div className="mb-6 bg-orange-50 border border-orange-200 text-orange-800 p-4 rounded-xl flex items-center gap-3 shadow-sm text-xs font-bold z-10 relative">
                ...
                <nav className="space-y-1">
                  {menuData.categories.map(cat => (
                    <button
                      ...
                      className="guest-sidebar-item font-bold text-sm"
                    >
          ...
          <div className={`col-span-1 ${
            ...
            {activeTab === 'menu' && (
              <div className="animate-in fade-in duration-200 space-y-6">
                ...
                {!searchQuery && (
                  <div className="flex md:hidden overflow-x-auto gap-2 pb-2 mb-4 scrollbar-hide -mx-4 px-4 sticky top-[68px] z-30 bg-[#f4f7f5]/90 backdrop-blur-sm py-2">
                    {menuData.categories.map(cat => (
                      <button 
                        key={cat.id}
                        onClick={() => scrollToCategory(cat.id)}
                        className="whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold bg-white border border-gray-200 text-gray-700 active:bg-green-50 active:text-[#16a34a] active:border-green-300 flex items-center gap-1 shadow-sm transition-all"
                      >
                ...
                <div className="space-y-8">
                  {groupedMenu.length > 0 ? (
                    groupedMenu.map((category) => (
                      <div id={`category-${category.id}`} key={category.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-0 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
                          {category.items.map((item) => {
                            ...
                            return (
                              <div key={item.name} className="p-5 flex gap-4 bg-white transition-colors hover:bg-slate-50/30">
              ...
              <div className="animate-in fade-in duration-200 space-y-6">
                ...
                {batches.length === 0 ? (
                  <div className="text-center py-20 bg-white border border-gray-200 rounded-2xl shadow-sm">
                    ...
                    <button 
                      onClick={() => handleTabChange('menu')} 
                      className="mt-6 guest-btn-add px-5 py-2.5 rounded-xl text-xs font-bold"
                    >
                      Browse Menu
                    </button>
                  </div>
                  ...
                  <div className="space-y-6">
                    {batches.map((batch) => (
                      <div 
                        key={batch.id} 
                        className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm relative overflow-hidden"
                      >
                        ...
                        } text-white font-black text-[9px] px-3 py-1 rounded-bl-xl uppercase tracking-wide`}>
                          {batch.status}
                        </div>
                            ...
                            <li key={idx}>
                              <div className="flex justify-between items-start">
                                ...
                                <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full uppercase ${
                                  item.status === 'served' ? 'bg-gray-100 text-gray-500 border border-gray-200' :
            ...
            {activeTab === 'bill' && (
              <div className="animate-in fade-in duration-200 space-y-6">
                ...
                {isBillRequested && (
                  <div className="mb-6 bg-green-50 border border-green-200 text-green-800 p-4 rounded-xl flex items-center gap-3 shadow-sm animate-in fade-in zoom-in">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm font-mono text-xs mb-6">
                  ...
                  <div className="p-4 space-y-4 pb-6 border-b border-gray-200 border-dashed min-h-[100px] text-slate-700">
                    {batches.length === 0 ? (
                      <p className="text-gray-400 text-center font-sans py-8">No orders placed in this session.</p>
                    ) : (
                      batches.map((batch) => (
                        <div key={batch.id} className="space-y-1.5">
                          ...
                          {batch.items.map((item, idx) => (
                            <div key={idx} className="flex justify-between items-start text-xs text-slate-650">
                              <div className="w-3/4">
                ...
                <button 
                  className={`w-full font-black py-4 text-base rounded-xl transition-all ${
                    isBillRequested || subtotal === 0 
                      ? 'bg-slate-100 text-gray-400 border border-slate-200 cursor-not-allowed' 
                      : 'guest-btn-add text-white active:scale-95'
                  }`}
            ...
            <div className="hidden lg:block lg:col-span-1 sticky top-24">
              <div className="guest-cart-panel p-5 space-y-4">
                ...
                {cartItems.length === 0 ? (
                  ...
                  <div className="space-y-4 max-h-[40vh] overflow-y-auto pr-1 scrollbar-glass">
                    {cartItems.map((item) => (
                      <div key={item.id} className="p-3 border border-gray-100 rounded-xl bg-slate-50/50">
                        <div className="flex justify-between items-start gap-1">
                          ...
                          <div className="guest-stepper h-[24px]">
                            <button 
                              ...
                              className="guest-stepper-btn px-1.5"
                            >-</button>
                        ...
                        <input
                          ...
                          className="guest-input-search w-full text-[10px] rounded-lg p-2 mt-2 font-semibold"
                        />
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="border-t border-gray-200/80 pt-4 space-y-4">
                    <div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="guest-input-search w-full rounded-lg p-2 text-[10px] font-semibold resize-none"
                      />
                    </div>
                    ...
                    <button 
                      ...
                      className="w-full guest-btn-add text-white font-bold py-3 text-xs rounded-xl transition-all active:scale-95"
                    >
                      Place Order
                    </button>
                  </div>
      ...
      <div className="fixed bottom-[96px] md:bottom-6 right-6 md:right-8 flex flex-col-reverse gap-4 pointer-events-none z-40">
        ...
        {activeTab === 'menu' && (
          <button
            ...
            className="pointer-events-auto w-16 h-16 btn-premium-green rounded-full flex items-center justify-center shadow-2xl active:scale-95 transition-all border-4 border-[#f4f7f5] cart-fab-glow relative"
            title="View Cart"
          >
        ...
        <button
          ...
          className="pointer-events-auto w-16 h-16 btn-premium-amber rounded-full flex items-center justify-center shadow-2xl active:scale-95 transition-all border-4 border-[#f4f7f5] animate-pulse"
          title="Call Staff"
        >
          <BellRing size={28} />
        </button>
          ...
          <div className="bg-white border-t border-gray-200 rounded-t-3xl w-full max-w-[480px] p-6 max-h-[85vh] overflow-y-auto z-10 shadow-[0_-8px_32px_rgba(0,0,0,0.15)] flex flex-col relative animate-in slide-in-from-bottom-24 duration-300">
            ...
            <div className="flex justify-between items-center mb-6 pb-3 border-b border-gray-150">
              ...
              <button 
                ...
                className="p-2 rounded-full bg-slate-100 hover:bg-slate-200 text-gray-500"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto scrollbar-glass mb-6">
              {cartItems.map((item) => (
                <div key={item.id} className="p-4 border border-gray-100 rounded-xl bg-slate-50/50">
                  <div className="flex justify-between items-start gap-2">
                    <div>
                  ...
                  <input
                    type="text"
                    placeholder="Special requests (e.g. no onions, make it spicy)..."
                    value={item.notes}
                    onChange={(e) => updateNotes(item.id, e.target.value)}
                    className="guest-input-search w-full text-xs rounded-lg p-2.5 mt-3 font-semibold"
                  />
                </div>
              ))}
            </div>
            ...
            <div className="bg-slate-50 border border-gray-100 rounded-xl p-4 mb-6">
              ...
              <textarea
                placeholder="e.g. Bring extra plates, serve desserts at the end..."
                value={generalComment}
                onChange={(e) => setGeneralComment(e.target.value)}
                rows={2}
                className="guest-input-search w-full rounded-lg p-2.5 text-xs font-semibold resize-none"
              />
            </div>
            ...
            <div className="border-t border-gray-200/80 pt-4 space-y-4">
              ...
              <button 
                onClick={handlePlaceOrder}
                className="w-full guest-btn-add text-white font-bold py-4 text-sm rounded-xl transition-all active:scale-95"
              >
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm border border-gray-250 shadow-2xl z-10 relative text-slate-800">
            ...
            <div className="flex gap-3">
              <button 
                className="flex-1 py-2.5 border border-gray-300 rounded-xl font-bold text-gray-650 hover:bg-slate-50 transition-colors text-xs" 
                onClick={() => setShowConfirm(false)}
              >
                Cancel
              </button>
              <button 
                className="flex-1 py-2.5 guest-btn-add text-white rounded-xl font-bold transition-all text-xs" 
                onClick={confirmBill}
              >
                Confirm Request
              </button>
      ...
      <nav className="fixed md:hidden bottom-0 left-0 w-full bg-white border-t border-gray-200/80 px-6 py-3 flex justify-between items-center z-40 shadow-md">
        ...
        <button 
          onClick={() => handleTabChange('menu')}
          className={`flex flex-col items-center gap-1 bg-transparent p-1.5 transition-all ${
            activeTab === 'menu' ? 'text-[#16a34a] scale-105 font-bold' : 'text-gray-400 hover:text-gray-550'
          }`}
        >
        ...
        <button 
          onClick={() => handleTabChange('orders')}
          className={`flex flex-col items-center gap-1 bg-transparent p-1.5 transition-all ${
            activeTab === 'orders' ? 'text-[#16a34a] scale-105 font-bold' : 'text-gray-400 hover:text-gray-550'
          }`}
        >
        ...
        <button 
          onClick={() => handleTabChange('bill')}
          className={`flex flex-col items-center gap-1 bg-transparent p-1.5 transition-all ${
            activeTab === 'bill' ? 'text-[#16a34a] scale-105 font-bold' : 'text-gray-400 hover:text-gray-550'
          }`}
        >
