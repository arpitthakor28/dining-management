...
import { ArrowLeft, UtensilsCrossed, Send } from 'lucide-react';
...
export default function OrderStatus() {
    ...
    return (<div className="pb-36 pt-4 px-4 bg-gray-50 h-full relative" style={{ minHeight: '100vh' }}>
      ...
      <div className="space-y-4">
        {batches.length === 0 ? (<div className="text-center py-20 text-gray-400">
            ...
            <button onClick={() => navigate(`/table/${tableId}/menu`)} className="mt-4 px-4 py-2 border border-primary text-primary rounded-lg font-bold">Menu</button>
          </div>) : (batches.map((batch) => (<div key={batch.id} className={`card border-l-4 ${batch.status === 'served' ? 'border-l-gray-400' :
                batch.status === 'ready' ? 'border-l-green-500' :
                    batch.status === 'preparing' ? 'border-l-orange-500' : 'border-l-blue-500'} p-4 shadow-sm relative overflow-hidden bg-white`}>
              
              <div className={`absolute top-0 right-0 ${batch.status === 'served' ? 'bg-gray-400' :
                batch.status === 'ready' ? 'bg-green-500' :
                    batch.status === 'preparing' ? 'bg-orange-500' : 'bg-blue-500'} text-white font-bold text-xs px-3 py-1 rounded-bl-lg flex items-center gap-1 uppercase`}>
                {batch.status}
              </div>
              ...
              <ul className="text-[13px] space-y-3 mb-4 text-gray-700 font-medium">
                {batch.items.map((item, idx) => (<li key={idx} className="border-b border-gray-50 pb-2">
                    <div className="flex justify-between items-start">
                      <span className="w-3/4 leading-tight">{item.qty}x {item.name}</span>
                      <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full uppercase ${item.status === 'served' ? 'bg-gray-100 text-gray-500' :
              ...
              {/* Progress bar */}
              {batch.status === 'preparing' && (<div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
      ...
      {batches.length > 0 && (<div className="mt-8 bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          ...
          <div className="flex gap-2">
            <input type="text" placeholder="Type message here..." value={commentText} onChange={(e) => setCommentText(e.target.value)} className="flex-1 bg-gray-50 border border-gray-200 rounded-lg py-2 px-3.5 outline-none focus:border-primary text-sm font-medium"/>
            <button onClick={handleSendComment} className="bg-primary hover:bg-primary-dark text-white font-bold p-2.5 rounded-lg transition-all flex items-center justify-center shadow-md active:scale-95">
              <Send size={18}/>
            </button>
          </div>
        </div>)}
